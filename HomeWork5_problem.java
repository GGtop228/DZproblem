...
public class HomeWork5 {
    ...
    */
    ...
    public static class Mage extends Person{
        ...
        private int protection= getProtection();
        Mage (String name){
            super("Mage",15, 100 );
            this.name=name;
        ...
        @Override
        String announce() {
            return (name+"Person"+ getName()+"имеет характеристики: "+getHealth()+"здоровья и"+getProtection()+"защиты");
        }
        @Override
        void takeDamage(Integer damage) throws PersonDead, Exception {
            damage= getHealth() - (damage - getProtection() - getHealth() % 10);
            //  Я НЕ ПОИНИМАЮ КАК ВЫЗАВТЬ SUPER CLASS    (super.takeDamage(что сюда ложить?);) я пишу так и у меня ошибка
    ...
    public static class Archer extends Person {
        ...
        void takeDamage(Integer damage) throws Exception, PersonDead {
            ...
            damage = getHealth()- (damage - getProtection() - getHealth() % 10);
            super.takeDamage(damage);// ТУТ ТОЖЕ САМОЕ 
